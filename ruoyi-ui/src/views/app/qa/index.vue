...
<script>
import { listQa, getQa, delQa, addQa, updateQa } from "@/api/app/qa";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除法律问答信息编号为"' + legalQaIds + '"的数据项？').then(function() {
        return delQa(legalQaIds);
      }).then(() => {
        this.getList();
